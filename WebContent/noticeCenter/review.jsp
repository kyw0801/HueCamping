...
<%-- 본문 --%>
    ...
    <div class="notice_cont">
     <table>
      ...
      <tr>
        ...
        <td class="product 1">
         <a href="#">
          <c:set var="fullPath" value="/HueCamping/upload/${r.board_file1}" />
           <img src="${fullPath}" alt="null" style="max-width: 70px; max-height: 70px;">
         </a>
        </td> 
            
        <td align="center">
         <a href="noticeCenter_review_view.net?board_no=${r.board_no}&page=${page}&state=cont">
          ${r.board_title}
         </a>
        </td>
        <td align="center">${r.board_name}</td>
        <td align="center">${r.board_date}</td>
  ...
  <div id="bList_paging">
   ...
   <c:if test="${(empty find_field) && (empty find_name)}"> <%--검색필드와 검색어가 없는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="noticeCenter_review.net?page=${page-1}">[이전]</a>&nbsp;
     </c:if>
     ...
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a != page}"> <%--현재 쪽번호가 선택 안 된 경우--%>
        <a href="noticeCenter_review.net?page=${a}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>
    ...
    <c:if test="${page < maxpage}">
     <a href="noticeCenter_review.net?page=${page+1}">[다음]</a>
    </c:if>
   </c:if>
   ...
   <c:if test="${(!empty find_field) && (!empty find_name)}"> <%--검색필드와 검색어가 있는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="noticeCenter_review.net?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
     </c:if> 
   ...
   <%--현재 쪽번호 출력 --%>
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a != page}"> <%--현재 쪽번호가 선택 안 된 경우--%>
        <a href="noticeCenter_review.net?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>      
    ...
    <c:if test="${page < maxpage}">
     <a href="noticeCenter_review.net?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
    </c:if>
   </c:if> 
   </div>
      ...
      <tr>
       <td>
        <div class="insert_cont">
         <input class="review_write" type="button" value="글쓰기" onclick="location='noticeCenter_review_write.net?page=${page}';">
        </div>
  
    <%-- 검색 버튼 --%>                        
       <div class="productQnA_searchBar">
        <form method="get" action="noticeCenter_review.net" >
